...
export function WitnessExperience({ offeringId, fallbackOffering }: { offeringId:string; fallbackOffering:OfferingPackageV0_2 }) {
  ...
  if (state === "declined") {
    return <main className="witnessDeclined"><X aria-hidden="true" /><p className="instrumentEyebrow">LOCAL DECLINE · NOT TRANSMITTED</p><h1>The Locket remains closed.</h1><p>No acceptance, refusal, receipt or delivery was recorded outside this browser.</p><Link className="instrumentSecondary" href="/studio"><ArrowLeft aria-hidden="true" /> Return to the Studio</Link></main>;
  }

  return (
    <main className={`witnessExperience state-${state}`}>
      <header className="instrumentTopbar witnessTopbar"><Link className="instrumentBrand" href="/"><ArrowLeft aria-hidden="true" /> Pinecœne <span>Encounter</span></Link><div className="artifactIdentity"><strong>{offering.resolution} · {offering.fixtureId}</strong><span>{offering.standing.replaceAll("_"," ")}</span></div><nav aria-label="Encounter navigation"><Link href="/studio">Studio</Link></nav></header>
