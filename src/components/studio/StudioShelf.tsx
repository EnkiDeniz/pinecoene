  ...
  return (
    <main className="studioShelf sketchesShelf">
      <section className="shelfIntro">
        <div><p className="instrumentEyebrow">SKETCHES · CURRENT STUDIES</p><h1>Two records.<br /><em>Two studies. One missing test.</em></h1></div>
        ...
        {artifacts.map((artifact, index) => (
          <article className="artifactCard" key={artifact.manifest.fixtureId}>
            ...
            <div className="artifactCardCopy">
              ...
              <Link className="instrumentPrimary" href={`/sketches/${artifact.manifest.fixtureId}`}>Enter the sketch <ArrowRight aria-hidden="true" /></Link>
            </div>
          </article>
        ))}
      </section>
      ...
      <section className="localStudies">
        <div className="sectionRuleHeading"><div><p className="instrumentEyebrow">SESSION STUDIES</p><h2>Browser-local forks</h2></div><Link className="instrumentSecondary" href="/use"><GitFork aria-hidden="true" /> Fork a specimen</Link></div>
        {studies.length ? <div className="studyRows">{studies.map((study) => <Link key={study.studyId} href={`/sketches/${study.fixtureId}?study=${study.studyId}`}><span><Flask aria-hidden="true" /> {study.studyId}</span><span>{study.address} · {study.resolution}</span><ArrowRight aria-hidden="true" /></Link>)}</div> : <div className="emptyStudy"><LockKey aria-hidden="true" /><p>No local studies yet. A fork stays bound to its fixture hash and never impersonates the canonical specimen.</p></div>}
      </section>
